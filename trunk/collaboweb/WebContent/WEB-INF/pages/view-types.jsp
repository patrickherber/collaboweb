...
<html>
<head>
	<title><bean:message key="application.title" /> &gt; <bean:message key="viewTypes.pageTitle" /></title>
...
<h2><bean:message key="viewTypes.pageTitle" /></h2>
...
<table width="500" class="list">
	<thead>
		<tr>
			<th width="18%"><img src="images/lang_en.gif" alt=""/></th>
			<th width="18%"><img src="images/lang_de.gif" alt=""/></th>
			<th width="18%"><img src="images/lang_fr.gif" alt=""/></th>
			<th width="18%"><img src="images/lang_it.gif" alt=""/></th>
			<th width="15%"><bean:message key="viewTypes.contentType"/></th>
			<th width="17%"></th>
		</tr>
	</thead>
	<tbody>
		<c:set var="bg" value="-1"/>
		<html:form action="/view-type.html" focus="labels(en)">
	 		<tr class="bg<c:out value="${bg}"/>">
				...
				<td><html:text property="contentType"/></td>
				<td>
		...
		<c:forEach var="item" items="${list}">
			<c:choose>
				<c:when test="${item.communityId == userInfo.communityId}">
					<html:form action="/view-type.html">
				 		<tr class="bg<c:out value="${bg}"/>">
							...
							<td><html:text name="item" property="contentType"/></td>
							<td>
								<html:hidden name="item" property="communityId"/>
								<html:hidden name="item" property="viewTypeId"/>
								<input type="hidden" name="submitted" value="true"/>
								<html:submit styleClass="button"><bean:message key="application.save"/></html:submit>
								<input type="button" class="button" onclick="return ask('<bean:message key="deleteQuestion.viewType"/>','<c:url value="view-type-delete.html?viewTypeId=${item.viewTypeId}"/>');" value="<bean:message key="application.delete"/>">
		   					</td>
						</tr>
					</html:form>
				</c:when>
