...
public class FileZipAction implements Action {
		...
		if (files != null && files.length > 0) {
			try {
				...
				response.addHeader("Content-Disposition", 
						"attachment; filename=\"collaboweb.zip\"");
				...
				for (long resourceId : files) {
					addToZip(zos, resourceManager.get(resourceId, user), "", 
							parameter, user);
				}		
				zos.close();
	...
	protected void addToZip(ZipOutputStream zos, Resource file, 
			String path, String attributeId, User user) throws Exception {
		if (file != null) {
			if (file.getTypeId() == 5) {
				BinaryAttribute ba = resourceManager.readAttribute(
						file.getResourceId(), attributeId, user);
            	addZipEntry(path, file.getName(), 
            			file.getUpdateDate().getTime(), zos);
	            int read = 0; 
				...
				/*
            	addZipEntry(path, file.getName() + ".html", 
            			file.getUpdateDate().getTime(), zos);
                StringBuffer sb = new StringBuffer();
                sb.append("<html><head><title>").append(file.getName())
                  .append("</title></head><body>").append(file.getDescription())
                  .append("</body></html>");
	...
	private void addZipEntry(String path, String fileName, long time, 
			ZipOutputStream zos) throws IOException {
		int collisionCounter = 0;
        do {
            try {
                ZipEntry entry = new ZipEntry(getZipEntryName(path, fileName,
                		collisionCounter)); 
                entry.setTime(time);
	...
	private String getZipEntryName(String path, String fileName, int collisionCounter) {
		...
		if (collisionCounter == 0) {
			...
			if (lastPoint == -1) {
				return new StringBuffer(path).append(fileName).append("_")
					.append(collisionCounter).toString();
			} else {
				return new StringBuffer(path)
					.append(fileName.substring(0, lastPoint)).append("_")
					.append(collisionCounter)
					.append(fileName.substring(lastPoint)).toString();
			}
		}
	}
    
    private String toUsAscii(String value) {
    	String[] find = { "ä", "ü", "ö", "à", "è", "é", "ê", "ì", "ò", 
    			"ù", "ç", "ñ",  "Ä", "Ö", "Ü", "À", "È", "É", "Ê", "Ì", 
    			"Ò", "Ù", "Ñ", "\\", "/", ":", "*", "?", "\"", "<", ">", "|" };
    	String[] replace = { "ae", "ue", "oe", "a", "e", "e", "e", "i", "o", 
    			"u", "c", "n",  "Ae", "Oe", "Ue", "Ae", "E", "E", "E", "I", 
    			"O", "U", "N", "_", "_", "_", "_", "_", "_", "_", "_", "_" };
