...
public class IndexerManagerImpl implements IndexerManager, InitializingBean, DisposableBean {
	...
	private List<QueueEntry> indexQueue = 
		Collections.synchronizedList(new LinkedList<QueueEntry>());
	...
	public void setRepository(String repository) {
		...
		Assert.isTrue(file.isDirectory(), 
				"Repository is not a directory: " + repository);
	...
	/* (non-Javadoc)
	 * @see ch.arpage.collaboweb.services.IndexerManager#index(ch.arpage.collaboweb.model.Resource, ch.arpage.collaboweb.model.User)
	 */
	...
	/* (non-Javadoc)
	 * @see ch.arpage.collaboweb.services.IndexerManager#search(ch.arpage.collaboweb.model.SearchQuery, ch.arpage.collaboweb.model.User)
	 */
	...
	public List search(SearchQuery searchQuery, User user) throws Exception {
        ...
        try {
		    ...
		    if (addFilterFor("name", searchQuery.getName(), query, 
    				BooleanClause.Occur.SHOULD, true)) {
                timeSearch = false;
		    }
		    if (StringUtils.hasText(searchQuery.getQuickSearch())) {
		    	Collection<String> fieldNames =
		    		is.getIndexReader().getFieldNames(IndexReader.FieldOption.ALL);
		    	if (fieldNames != null) {
			    	...
			    	query.add(MultiFieldQueryParser.parse(searchQuery.getQuickSearch(),
			    			fields, occur, new StandardAnalyzer()), 
			    			BooleanClause.Occur.SHOULD);
		    	}
		    }
		    if (searchQuery.getTypeId() != 0) {
			    if (addFilterFor("typeId", 
			    		Integer.toString(searchQuery.getTypeId()), 
			    		query, BooleanClause.Occur.SHOULD, true)) {
	                timeSearch = false;
			    }
		    }
		    if (StringUtils.hasText(searchQuery.getFree())) {
		    	query.add(new QueryParser("", 
		    			new StandardAnalyzer()).parse(searchQuery.getFree()), 
		    			BooleanClause.Occur.SHOULD);
		    }
            ...
            for (String name : fields.keySet()) {
            	...
            	if (StringUtils.hasText(value)) {
            		if (addFilterFor(name, value, query, 
            				BooleanClause.Occur.SHOULD, true)) {
                        timeSearch = false;
        		    }
            	}
            ...
            if (timeSearch && filter != null) {
            	query.add(new WildcardQuery(new Term("name", "*")), 
	...
	private RangeFilter getTimeFilter(SearchQuery searchQuery, RangeFilter filter) {
		...
		if (date != null) {
		    ...
		    case -1:
		        ...
		        filter = new RangeFilter("modified", 
		                DateTools.dateToString(cal.getTime(), 
		                		DateTools.Resolution.DAY), 
		        		DateTools.dateToString(from.getTime(), 
		        				DateTools.Resolution.DAY), 
		                true, true);
		    	break;
		    case 1:
		        filter = new RangeFilter("modified", 
		        		DateTools.dateToString(cal.getTime(), 
		        				DateTools.Resolution.DAY), 
		                DateTools.dateToString(Calendar.getInstance().getTime(), 
		                		DateTools.Resolution.DAY), 
		                true, true);
		    	break;
		    default:
		    	String str = DateTools.dateToString(cal.getTime(),
		    			DateTools.Resolution.DAY);
    ...
    private boolean addFilterFor(String fieldName, String value, 
    		BooleanQuery query, BooleanClause.Occur occur, 
    		boolean allowWildcard) throws Exception {
        if (StringUtils.hasText(value)) {
        	if (!allowWildcard || value.indexOf('*') == -1) {
        		...
        		query.add(new WildcardQuery(
        				new Term(fieldName, value.toLowerCase())), occur);
        	}
            return true;
        }
        return false;
	...
	private void indexResource(Resource resource, User user) {
		...
		try {
			deleteFromIndex(resource.getResourceId(), resource.getCommunityId(), 
					directory);
			try {
		...
		for (Attribute attribute : resource.getResourceType().getAttributes()) {
			if (attribute.getSearchFieldType() != 0) {
				...
				if (attribute.getDataType() != Model.DATA_TYPE_BINARY) {
					doc.add(new Field(attribute.getIdentifier(), 
							(String) resourceAttribute.getValue(), 
							Field.Store.YES, 
	...
	private void indexBinaryAttribute(Resource resource, Attribute attribute, Document doc) {
		...
		try {
			ba = resourceDao.readAttribute(
					resource.getResourceId(), attribute.getIdentifier());
			String contentType = ba.getContentType();
			if (contentType != null) {
				doc.add(new Field("contentType", contentType,
						Field.Store.YES, Field.Index.UN_TOKENIZED));
			}
		    addContent(resource.getResourceId(), 
		    		attribute.getIdentifier(), ba, doc);
		} catch (Exception e) {
			LOG.error("Exception parsing file: " + 
					resource.getResourceId(), e);
		} finally {
			if (ba != null) {
				try { ba.getInputStream().close(); } 
				catch (Exception e) {}
			}
		}
	}
	...
	private void addField(String name, Date value, Document doc) {
		...
		addField(name, DateTools.dateToString(value, 
				DateTools.Resolution.MINUTE), doc);
	}
	...
	/* (non-Javadoc)
	 * @see ch.arpage.collaboweb.services.IndexerManager#deleteFromIndex(long, ch.arpage.collaboweb.model.User)
	 */
	public void deleteFromIndex(long resourceId, User user) {
        Directory directory = null;
        try {
        	directory = getIndexDirectory(user.getCommunityId(), false);
        	deleteFromIndex(resourceId, 
        			new Term("resourceId", Long.toString(resourceId)), directory);
        } catch (Exception e) {
    ...
    private String parsePDFFile(InputStream stream, long rid) throws Exception {
        ...
        try {
            ...
            if (document.isEncrypted()) {
                LOG.warn("Could not parse PDF File " + rid + 
                		" since the document is encrypted");
            } else {
            ...
            LOG.error("EOF Exception parsing PDF Document [" + title + "]: " 
                    + rid);
            return "";
        } finally {
            if (document != null) {
    ...
    private void addContent(long resourceId, String fieldName, 
    		BinaryAttribute ba, Document doc) throws Exception {
    	String contentType = ba.getContentType();
        if ("application/msword".equals(contentType)) {
            try {
                doc.add(new Field(fieldName, parseWorldFile(ba.getInputStream()), 
                		Field.Store.NO, Field.Index.TOKENIZED));
            } catch (FastSavedException fse) {
                LOG.error("Document 'fast-saved' indexing not supported: " + 
                		resourceId);
                doc.add(new Field("contents", new InputStreamReader(ba.getInputStream())));
            } catch (Exception e) {
                LOG.error("Exception indexing Word file: " + resourceId, e);
                // The POI Word Parser still does not support all document types
                // Try with a normal text parser
                doc.add(new Field(fieldName, 
                		new InputStreamReader(ba.getInputStream())));
            }
        } else if (contentType != null && contentType.indexOf("pdf") != -1) {
            doc.add(new Field(fieldName, parsePDFFile(ba.getInputStream(), resourceId), 
            		Field.Store.NO, Field.Index.TOKENIZED));
            ...
            doc.add(new Field(fieldName, parser.getContents(ba.getInputStream()), 
            		Field.Store.NO, Field.Index.TOKENIZED));
        } else if ("application/vnd.ms-excel".equals(contentType)) {
        	try {
        		doc.add(new Field(fieldName, parseExcelFile(ba.getInputStream()), 
            		Field.Store.NO, Field.Index.TOKENIZED));
	    	} catch (FileNotFoundException e) {
				LOG.warn("Could not index Excel file " + resourceId + " " +
						"probably vervsion prior Excel 97");
			}
        } 
    }
	...
	private Directory getDirectory(long cid, String name, boolean create) throws Exception {
        ...
        if (!communityRoot.isDirectory()) {
            if (!communityRoot.mkdir()) {
                LOG.error("Could not create community directory: " 
                        + communityRoot.getPath());
                return null;
            }
        }
