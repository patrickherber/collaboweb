...
/**
 * Struts action used to delete an attribute validation object.
 *
 * @author <a href="mailto:patrick@arpage.ch">Patrick Herber</a>
 */
public class ValidationDeleteAction extends AbstractSecureAction {
	...
	protected ActionForward executeAction(ActionMapping mapping, ActionForm form, 
		...
		if (attributeId != 0 && validationTypeId != 0) {
			try {
				resourceTypeManager.deleteValidation(attributeId, 
						validationTypeId, user);
				addSessionMessage(request, "message.validation.deleted");
				return redirect(request, response, mapping, 
						Constants.SUCCESS_KEY, "?attributeId=" + attributeId);
			} catch (Exception e) {
				addGlobalError(request, e);
			}
		}
		return forward(mapping, Constants.SUCCESS_KEY, 
				"?attributeId=" + attributeId, false);
	}
}
